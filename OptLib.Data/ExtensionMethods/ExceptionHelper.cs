...
namespace OptLib.Data.ExtensionMethods
{
    ...
    public static class ExceptionHelper
    {
        ...
        public static Exception CreateFromDbUpdateException(DbUpdateException dbUpdateException)
        {
            ...
            {
                if (entry.State != System.Data.Entity.EntityState.Deleted && entry.CurrentValues != null)
                {
                    var obj = entry.CurrentValues.ToObject();
                    var type = obj.GetType();
                    var propertyNames = entry.CurrentValues.PropertyNames.Where(x => inner.Message.Contains(x)).ToList();
                    ...
                    message += "Entry " + i++ + " " + type.Name + ": " + string.Join("; ", propertyNames.Select(x =>
                        string.Format("'{0}' = '{1}'", x, entry.CurrentValues[x])));
